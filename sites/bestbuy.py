...
class BestBuy:
    def __init__(self,task_id,status_signal,image_signal,product,profile,proxy,monitor_delay,error_delay):
        ...
        self.status_signal.emit({"msg":"Got TAS Data Back","status":"normal"})
    ...
    def monitor(self):
        ...
        while True:
            ...
            try:
                ...
                self.status_signal.emit({"msg":"Loading Product Page 1","status":"normal"})
                if r.status_code == 200:
                    doc = lxml.html.fromstring(r.text)
                    if not image_found:
                        self.sku_id = doc.xpath('//span[@class="product-data-value body-copy"]/text()')[1].strip()
                        self.status_signal.emit({"msg":"SKU ID " + self.sku_id,"status":"normal"})
    ...
    def check_stock(self):
        ...
        while True:
            ...
            try:
                ...
                self.status_signal.emit({"msg":"Checking Stock 1 " + url,"status":"normal"})
                r = self.session.get(url,headers=headers)
                self.status_signal.emit({"msg":"Checking Stock 1","status":"normal"})
    ...
    def atc(self):
        ...
        while True:
            ...
            try:
                self.status_signal.emit({"msg":"Enter add to cart " + self.product,"status":"normal"})
                r = self.session.post("https://www.bestbuy.com/cart/api/v1/addToCart",json=body,headers=headers)
                self.status_signal.emit({"msg":"Added To Cart " + r.status_code,"status":"normal"})
    ...
    def submit_order(self):
        ...
        body = {"browserInfo":{"javaEnabled":False,"language":"en-US","userAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36","height":"1057","width":"1057","timeZone":"240","colorDepth":"24"}}
        while True:
