            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if not image_found:
                        ...
                        image_found = True
                        self.status_signal.emit({"msg":"image found","status":"normal"})
                    price = float(doc.xpath('//span[@itemprop="price"]/@content')[0])
                    self.status_signal.emit({"msg":str(price),"status":"normal"})
                    #self.status_signal.emit({"msg":r.text,"status":"normal"})
                    if "Add to cart" in r.text:
                        self.status_signal.emit({"msg":"add to cart","status":"normal"})
                        if self.max_price !="":
                            if float(self.max_price) < price:
                                self.status_signal.emit({"msg":"Waiting For Price Restock 1","status":"normal"})
                                self.session.cookies.clear()
                                time.sleep(self.monitor_delay)
                                continue
                        offer_id = json.loads(doc.xpath('//script[@id="item"]/text()')[0])["item"]["product"]["buyBox"]["products"][0]["offerId"]
                        self.status_signal.emit({"msg":"offer_id","status":"normal"})
                        return product_image, offer_id
                    self.status_signal.emit({"msg":"Waiting For Restock 2","status":"normal"})
                    self.session.cookies.clear()
                    time.sleep(self.monitor_delay)
                else:
    ...
    def atc(self,offer_id):
        ...
        while True:
            ...
            try:
                ...
                self.status_signal.emit({"msg":str(r.status_code),"status":"normal"})
                self.status_signal.emit({"msg":r,"status":"error"})
            ...
            try:
                ...
                if r.status_code == 201:
                    ...
                    if json.loads(r.text)["message"] == "Item is no longer in stock.":
                        self.status_signal.emit({"msg":"Waiting For Restock 3","status":"normal"})
                        time.sleep(self.monitor_delay)
                    else:
